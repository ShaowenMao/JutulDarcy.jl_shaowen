#!/bin/bash
# Submit only canonical manifest tasks 5:7 for sandpc_ycap50_v1.
...
full_walltime="${GOM_SANDPC_FULL_WALLTIME:-1-00:00:00}"
[[ "$full_walltime" =~ ^([0-9]+-)?[0-9]{1,2}:[0-9]{2}:[0-9]{2}$ ]] ||
    {
        echo "Invalid GOM_SANDPC_FULL_WALLTIME: $full_walltime" >&2
        exit 1
    }
...
submit_job --kill-on-invalid-dep=yes --dependency="afterok:$full_job" \
    ...
    "$scripts/gom_step62_sandpc_ycap50_vtu.sbatch"
...
submit_job --kill-on-invalid-dep=yes --dependency="afterok:$vtu_job" \
    ...
    "$scripts/gom_step62_sandpc_ycap50_archive.sbatch"
archive_job="$submitted_job_id"

timestamp=$(date -u +%Y%m%dT%H%M%SZ)
receipt="$gom_root/submissions/gom_step62_sandpc_ycap50_${timestamp}.txt"
