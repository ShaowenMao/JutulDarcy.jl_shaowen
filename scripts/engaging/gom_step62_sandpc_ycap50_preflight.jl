...
length(ARGS) == 9 || error(
    "Usage: gom_step62_sandpc_ycap50_preflight.jl COMMON_MAT SPECIFIC_MAT " *
...
get(common_metadata, "physics_profile", "") ==
    GoMStep62SandPcYCap50Contract.PHYSICS_PROFILE ||
...
get(common_metadata, "fixed_sgr_file", "") ==
    "faultPermSGR_Gupper87lyr_predictMesh_youngerkxx50_v1.mat" ||
...
get(modification, "schema", "") == "gom_fixed_sgr_modification_v1" ||
...
))) == GoMStep62SandPcYCap50Contract.EXPECTED_BASE_REGIONS ||
...
setup = simulate_mrst_case(
    ...
    hysteresis_s_min =
        GoMStep62SandPcYCap50Contract.EXPECTED_HYSTERESIS_S_MIN,
    use_mrst_transmissibility = false,
    fault_saturation_domain_mode = "input",
    fault_pc_entry_treatment = "plateau",
    fault_pc_entry_sg_max =
        GoMStep62SandPcYCap50Contract.EXPECTED_PC_ENTRY_SG_MAX,
...
diagnostics =
    GoMStep62SandPcYCap50Contract.validate_assembled_case(setup)
...
base_counts = [
    ...
    for region in 1:GoMStep62SandPcYCap50Contract.EXPECTED_BASE_REGIONS
]
...
open(summary_path, "w") do io
    ...
    println(io, "physics_profile=$(GoMStep62SandPcYCap50Contract.PHYSICS_PROFILE)")
    ...
    println(
        io,
        "resolution_slices=$(GoMStep62SandPcYCap50Contract.EXPECTED_SLICES)"
    )
    println(io, "cells=$(GoMStep62SandPcYCap50Contract.EXPECTED_CELLS)")
    println(
        io,
        "fault_cells=$(GoMStep62SandPcYCap50Contract.EXPECTED_FAULT_CELLS)"
    )
    println(
        io,
        "stratigraphy_cells=" *
        string(GoMStep62SandPcYCap50Contract.EXPECTED_STRATIGRAPHY_CELLS)
    )
    println(
        io,
        "base_saturation_regions=" *
        string(GoMStep62SandPcYCap50Contract.EXPECTED_BASE_REGIONS)
    )
    println(io, "base_region_cell_counts=$(join(base_counts, ','))")
    println(
        io,
        "explicit_predict_regions=" *
        string(GoMStep62SandPcYCap50Contract.EXPECTED_FAULT_REGIONS)
    )
    println(
        io,
        "drainage_saturation_regions=" *
        string(GoMStep62SandPcYCap50Contract.EXPECTED_DRAINAGE_REGIONS)
    )
    println(
        io,
        "total_sgof_tables=" *
        string(GoMStep62SandPcYCap50Contract.EXPECTED_TOTAL_SGOF_TABLES)
    )
    println(io, "hysteresis_active=true")
    println(
        io,
        "hysteresis_s_min=" *
        string(GoMStep62SandPcYCap50Contract.EXPECTED_HYSTERESIS_S_MIN)
    )
    println(io, "fault_hysteresis=drainage_equivalent")
    println(io, "pc_curve_treatment=predict_fault_plateau_only")
    println(
        io,
    ...
    println(
        io,
        "host_pc_entry_pressures_Pa=" *
        join(
            [
                diagnostics.sgof[region][findfirst(
                    value -> isapprox(value, 0.001; rtol = 0,
                        atol = 10*eps(0.001)),
                    diagnostics.sgof[region][:, 1]
                ), 4]
                for region in (1, 6, 7, 8)
            ],
            ","
        )
    )
...
println(
    "STEP62_SANDPC_YCAP50_PREFLIGHT_PASS " *
