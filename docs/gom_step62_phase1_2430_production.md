...
9. Launch only the 2,380 complementary tasks. The task-set controller admits
   one checksum-pinned set at a time, promotes it to durable storage, and
   automatically continues at the next missing set. Final completion requires
   the 49 durable task sets to cover every canonical task exactly once.

The task-set controller is restartable and keeps the Slurm queue bounded.
Completed checksum-verified task sets are reused; the controller never
silently overwrites a durable result. Full-campaign completion is recorded
only after all 2,430 canonical tasks are present exactly once across the 49
verified durable task sets.
