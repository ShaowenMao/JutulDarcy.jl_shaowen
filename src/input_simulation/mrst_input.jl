...
# function simulate_predict_case2(fn; 
#         write_mrst = false,
#         plot_pressure = false,
#         kwarg...
#     )
...
#         if grid_dimension == 2
#             if dir == :x
#                 min_dir_cells = [Jutul.cell_index(wrapped_g, (1,  k))   for k in 1:nz]
#                 max_dir_cells = [Jutul.cell_index(wrapped_g, (nx, k))   for k in 1:nz]
#             elseif dir == :z
#                 min_dir_cells = [Jutul.cell_index(wrapped_g, (i,  1))   for i in 1:nx]
#                 max_dir_cells = [Jutul.cell_index(wrapped_g, (i, nz))   for i in 1:nx]
#             end
#         elseif grid_dimension == 3
#             if dir == :x
#                 min_dir_cells = vec([Jutul.cell_index(wrapped_g, (1,  j, k))   for j in 1:ny, k in 1:nz])
#                 max_dir_cells = vec([Jutul.cell_index(wrapped_g, (nx, j, k))   for j in 1:ny, k in 1:nz])
#             elseif dir == :y 
#                 min_dir_cells = vec([Jutul.cell_index(wrapped_g, (i, 1,  k))   for i in 1:nx, k in 1:nz])
#                 max_dir_cells = vec([Jutul.cell_index(wrapped_g, (i, ny, k))   for i in 1:nx, k in 1:nz])
#             elseif dir == :z 
#                 min_dir_cells = vec([Jutul.cell_index(wrapped_g, (i, j,  1))   for i in 1:nx, j in 1:ny])
#                 max_dir_cells = vec([Jutul.cell_index(wrapped_g, (i, j, nz))   for i in 1:nx, j in 1:ny])
#             end
#         end

#         bcells = vcat(min_dir_cells, max_dir_cells)
#         bpres  = vcat(fill(p_bc_pair[1], length(min_dir_cells)), fill(p_bc_pair[2], length(max_dir_cells)))
...
#         # Direct solver
#         p = cholesky(Symmetric(A, :U); check=false) \ rhs_bc
